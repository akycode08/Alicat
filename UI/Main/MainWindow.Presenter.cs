...
using PrecisionPressureController.UI.Features.Test;
using PrecisionPressureController.UI.Options;

namespace PrecisionPressureController.UI.Main
{
    /// <summary>
    /// Partial class MainWindow: интеграция с MainPresenter.
    /// Этот файл содержит связь между View (MainWindow) и Presenter (MainPresenter).
    /// </summary>
    public partial class MainWindow
    {
        ...
        ITerminalView? IMainView.TerminalView
        {
            get => _terminalForm;
            set => _terminalForm = value as TerminalWindow;
        }

        IGraphView? IMainView.GraphView
        {
            get => _graphForm;
            set => _graphForm = value as ChartWindow;
        }

        ITableView? IMainView.TableView
        {
            get => _tableForm;
            set => _tableForm = value as DataTableWindow;
        }
        ...
        // Метод доступен из других partial class файлов (MainWindow.Commands.cs)
        private void SaveSettingsToFile()
        {
            try
            {
                ...
                // Читаем RecentSessions и ChartWindow из существующего файла, если он есть
                ...
                if (System.IO.File.Exists(settingsPath))
                {
                    try
                    {
                        ...
                        // Сохраняем секцию ChartWindow из существующего файла
                        if (existingData.TryGetProperty("ChartWindow", out var graphFormProp))
                        {
                ...
                var settingsData = new
                {
                    General = new
                    {
                        PressureUnits = OptionsWindow.AppOptions.Current.PressureUnits,
                        TimeUnits = OptionsWindow.AppOptions.Current.TimeUnits,
                        PollingFrequency = OptionsWindow.AppOptions.Current.PollingFrequency
                    },
                    Device = new
                    {
                        PressureRamp = OptionsWindow.AppOptions.Current.PressureRamp,
                        MaxPressure = OptionsWindow.AppOptions.Current.MaxPressure,
                        MinPressure = OptionsWindow.AppOptions.Current.MinPressure
                    },
                    Control = new
                    {
                        MaxIncrement = OptionsWindow.AppOptions.Current.MaxIncrement,
                        MinIncrement = OptionsWindow.AppOptions.Current.MinIncrement
                    },
                    Communication = LoadCommunicationSettingsFromFile(),
                    AutoConnect = new
                    {
                        AutoConnectOnStartup = OptionsWindow.AppOptions.Current.AutoConnectOnStartup
                    },
                    RecentSessions = recentSessions ?? Array.Empty<string>(),
                    LastSaved = DateTime.Now
                };

                System.Diagnostics.Debug.WriteLine($"[SaveSettingsToFile] AutoConnectOnStartup = {OptionsWindow.AppOptions.Current.AutoConnectOnStartup}");
                ...
                // Parse to JsonObject to add ChartWindow section if it exists
                ...
                {
                    // Добавляем секцию ChartWindow из существующего файла
                    jsonDoc["ChartWindow"] = System.Text.Json.JsonSerializer.Deserialize<System.Text.Json.Nodes.JsonNode>(graphFormState.Value.GetRawText());
                    System.Diagnostics.Debug.WriteLine($"[SaveSettingsToFile] ChartWindow section preserved from existing file");
                }
        ...
        private void LoadSettingsFromFile()
        {
            try
            {
                ...
                var model = new OptionsWindow.AppOptions.Model();
                ...
                OptionsWindow.AppOptions.Current = model;
        ...
        {
            // Вызываем приватный метод ApplyOptionsToUi() из MainWindow.cs
        ...
        private void menuHelpAboutAlicat_Click(object? sender, EventArgs e)
        {
            ...
            string message = "Precision Pressure Controller\n\n" +
                           $"Version {version}\n\n" +
                           "Professional Windows application for controlling\n" +
                           "pressure controllers via RS-232.\n\n" +
                           "Features:\n" +
            ...
            MessageBox.Show(message, "About Precision Pressure Controller",
