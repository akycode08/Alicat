...
namespace PrecisionPressureController.UI.Features.Graph.Views
{
    ...
    public partial class ChartWindow
    {
        ...
        private PrecisionPressureController.Services.Sequence.SequenceService? _sequenceService;
        ...
        private ToolTip? _buttonToolTip; // ToolTip для кнопок управления
        // Используем _targetHandler из ChartWindow.HeaderFooter.cs

        /// <summary>
        /// Устанавливает SequenceService для синхронизации с фоновым выполнением
        /// </summary>
        internal void SetSequenceServiceInternal(PrecisionPressureController.Services.Sequence.SequenceService service)
        {
        ...
        private void InitializeGoToTarget()
        {
            ...
            // Создаём один ToolTip для всех кнопок (сохраняем как поле класса, чтобы не удалился сборщиком мусора)
            _buttonToolTip = new ToolTip();
            
            if (btnPlay != null)
            {
                btnPlay.Click += BtnPlay_Click;
                btnPlay.MouseEnter += (s, e) => btnPlay.BackColor = Color.FromArgb(92, 195, 96);
                btnPlay.MouseLeave += (s, e) => btnPlay.BackColor = Color.FromArgb(76, 175, 80);
                _buttonToolTip.SetToolTip(btnPlay, "Start / Play");
            }
            
            if (btnPauseTarget != null)
            {
                btnPauseTarget.Click += BtnPauseTarget_Click;
                btnPauseTarget.MouseEnter += (s, e) => 
                {
                    btnPauseTarget.BackColor = Color.FromArgb(55, 58, 68);
                    btnPauseTarget.FlatAppearance.BorderColor = Color.FromArgb(100, 105, 115);
                };
                btnPauseTarget.MouseLeave += (s, e) => 
                {
                    btnPauseTarget.BackColor = Color.FromArgb(42, 45, 53);
                    btnPauseTarget.FlatAppearance.BorderColor = Color.FromArgb(80, 85, 95);
                };
                _buttonToolTip.SetToolTip(btnPauseTarget, "Pause");
            }
            
            if (btnStop != null)
            {
                btnStop.Click += BtnStop_Click;
                btnStop.MouseEnter += (s, e) => 
                {
                    btnStop.BackColor = Color.FromArgb(55, 58, 68);
                    btnStop.FlatAppearance.BorderColor = Color.FromArgb(100, 105, 115);
                };
                btnStop.MouseLeave += (s, e) => 
                {
                    btnStop.BackColor = Color.FromArgb(42, 45, 53);
                    btnStop.FlatAppearance.BorderColor = Color.FromArgb(80, 85, 95);
                };
                _buttonToolTip.SetToolTip(btnStop, "Stop");
            }
            
            if (btnSkip != null)
            {
                btnSkip.Click += BtnSkip_Click;
                btnSkip.MouseEnter += (s, e) => 
                {
                    btnSkip.BackColor = Color.FromArgb(55, 58, 68);
                    btnSkip.FlatAppearance.BorderColor = Color.FromArgb(100, 105, 115);
                };
                btnSkip.MouseLeave += (s, e) => 
                {
                    btnSkip.BackColor = Color.FromArgb(42, 45, 53);
                    btnSkip.FlatAppearance.BorderColor = Color.FromArgb(80, 85, 95);
                };
                _buttonToolTip.SetToolTip(btnSkip, "Skip");
            }
        ...
        // SetTargetHandler уже определен в ChartWindow.HeaderFooter.cs
        ...
        private void BtnEdit_Click(object? sender, EventArgs e)
        {
            ...
            // Получаем тему из ChartWindow (используем internal поле)
            bool isDarkTheme = _isDarkTheme;

            // Открываем модальное окно
            using var modal = new SequenceEditorDialog(_targets, isDarkTheme);
        ...
        {
            // Используем _dataStore из ChartWindow (partial class)
