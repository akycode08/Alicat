...
namespace PrecisionPressureController.UI.Features.Graph.Views
{
    ...
    {
        // GetSettingsFilePath уже определен в ChartWindow.GoToTarget.cs как static, используем его
        ...
        private void SaveDisplaySettings()
        {
            try
            {
                ...
                // Создаем секцию ChartWindow если её нет
                if (!settingsData.ContainsKey("ChartWindow"))
                {
                    settingsData["ChartWindow"] = new JsonObject();
                }

                var graphFormObj = settingsData["ChartWindow"] as JsonObject;
                if (graphFormObj == null)
                {
                    graphFormObj = new JsonObject();
                    settingsData["ChartWindow"] = graphFormObj;
                }
        ...
        private void LoadDisplaySettings()
        {
            try
            {
                ...
                // Проверяем наличие секции ChartWindow
                if (!settingsData.TryGetPropertyValue("ChartWindow", out var graphFormNode) || graphFormNode is not JsonObject graphFormObj)
                {
                    System.Diagnostics.Debug.WriteLine($"[LoadDisplaySettings] ChartWindow section not found");
                    return;
                }
        ...
        private void LoadDurationSettings()
        {
            try
            {
                ...
                if (!settingsData.TryGetPropertyValue("ChartWindow", out var graphFormNode) || graphFormNode is not JsonObject graphFormObj)
                {
                    System.Diagnostics.Debug.WriteLine($"[LoadDurationSettings] ChartWindow section not found");
                    return;
                }
                ...
                if (durationObj.TryGetPropertyValue("SelectedIndex", out var selectedIndex) && selectedIndex != null)
                {
                    ...
                    // PaginationData определен в ChartWindow.cs, используем константу 6 (количество страниц)
